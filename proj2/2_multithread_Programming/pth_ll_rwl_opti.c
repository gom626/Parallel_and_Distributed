...
typedef struct lock_{
	...
	pthread_cond_t readl;
	pthread_cond_t writel;
	int readCnt;
	int writeCnt;
	int writing_state;
}lock;
...
int lock_init(lock* p_rwlock, const char* __not_used__)
{
    if (pthread_mutex_init(&p_rwlock->mutex, NULL) != 0) goto ERROR_MUTEX;
    if (pthread_cond_init(&p_rwlock->readl, NULL) != 0) goto ERROR_CV_READ;
    if (pthread_cond_init(&p_rwlock->writel, NULL) != 0) goto ERROR_CV_WRITE;

    p_rwlock->readCnt = p_rwlock->writeCnt = p_rwlock->writing_state = 0;

    return 0;

ERROR_CV_WRITE:
    pthread_cond_destroy(&p_rwlock->readl);
ERROR_CV_READ:
...
int lock_destroy(lock* p_rwlock)
{
    int fail = 0;
    fail = fail || pthread_mutex_destroy(&p_rwlock->mutex);
    fail = fail || pthread_cond_destroy(&p_rwlock->readl);
    fail = fail || pthread_cond_destroy(&p_rwlock->writel);
    if (fail)
        return -1;
    else
        return 0;
...
int lock_rdlock(lock* p_rwlock)
{
    pthread_mutex_lock(&p_rwlock->mutex);
    ++p_rwlock->readCnt;
    if (p_rwlock->writing_state)
        pthread_cond_wait(&p_rwlock->readl, &p_rwlock->mutex);
    pthread_mutex_unlock(&p_rwlock->mutex);

    return 0;
}

int lock_wrlock(lock* p_rwlock)
{
    pthread_mutex_lock(&p_rwlock->mutex);
    ++p_rwlock->writeCnt;
    if (p_rwlock->writing_state || p_rwlock->readCnt > 0)
        pthread_cond_wait(&p_rwlock->writel, &p_rwlock->mutex);
    p_rwlock->writing_state = 1;
    pthread_mutex_unlock(&p_rwlock->mutex);

    return 0;
}

int lock_unlock(lock* p_rwlock)
{
    pthread_mutex_lock(&p_rwlock->mutex);
    ...
    if (p_rwlock->writing_state) {
        ...
        if (p_rwlock->readCnt > 0) { 
            p_rwlock->writing_state = 0;
            pthread_mutex_unlock(&p_rwlock->mutex);
            pthread_cond_broadcast(&p_rwlock->readl);
        }
        else if (p_rwlock->writeCnt > 0) {
            pthread_mutex_unlock(&p_rwlock->mutex);
            pthread_cond_signal(&p_rwlock->writel);
        }
        else {
            p_rwlock->writing_state = 0;
            pthread_mutex_unlock(&p_rwlock->mutex);
