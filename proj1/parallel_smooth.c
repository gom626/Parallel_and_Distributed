...
void smooth(RGB* input, int blk_cnt, int blk_size)
{
	RGB* blk_ptr = input;
	int i,j,k;
	int avg;
	...
	for (i = 0; i < blk_cnt; ++i, blk_ptr += blk_size)
	{
		for (j = 0; j < blk_size; ++j)
		{
			...
			cnt=0;
			for(k=-1;k<1;k++){
				if(0<=(j+k)&&(j+k)<blk_size){
					cnt++;
					avg+=(int)blk_ptr[j+k].R;
				}
			}
			if(cnt!=0)
                blk_ptr[j].R = avg/cnt;
...
/*	for(i=0;i<size;i++){
	...
	smooth(data, data_block_cnt, width);
	merge_data(img.pixels, counts, displs, data, data_block_cnt);

	if (rank == root)
	{

		fnWritePPM("result_parallel_smooth.ppm", &img);
		fnClosePPM(&img);
	}
